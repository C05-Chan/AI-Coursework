...
class Full_NN(object):
    ...
    def __init__(self, X=24, HL=[256, 128, 64, 32], Y=24):  # a constructor for some default values.
        self.X = X  # inputs
        self.HL = HL  # hidden layers
        self.Y = Y  # outputs

        # setting up class variables for our inputs.
        ...
        for i in range(len(L) - 1):  # we want to be able go to the next layer up so we set one minus
            scale = np.sqrt(2.0 / L[i])  # Kaiming initialization scaling factor
            w = np.random.rand(L[i], L[i + 1]) * scale  # initialize weights with random values scaled appropriately
            W.append(w)  # add the new values to the array.
        self.W = W  # links the class variable to the current variable.
        Der = []  # initializes a derivative array. used to calculate back propagation.

        for i in range(len(L) - 1):  # same reason as above for every line
            d = np.zeros((L[i], L[
                i + 1]))  # we don't need random values, just to have them ready to be used. we fill up with zeros.
        ...
        for i in range(len(L)):  # We don't need to go +1. The outputs are straightforward.
            o = np.zeros(L[i])  # fills with zeros to be used later.
            out.append(o)
    ...
    def FF(self, x):  # Feed-Forward method
        ...
        self.out[
            0] = x  # begin the linking of outputs to the class variable for back propagation. (begin with the input layer.)
        for i, w in enumerate(self.W):  # go through (iterate) the network layers via the weights variable
            ...
            self.out[
                i + 1] = out  # pass the result to the class variable to preserve for later (when we do the back propagation).
    ...
    def train_nn(self, x, target, epochs, lr):  # training the network

        for i in range(epochs):  # training loop for as many epochs as specified

            S_errors = 0  # variable to carry the error to report to the user

            for j, input in enumerate(x):  # iterates through the training data and inputs
                t = target[j]
            ...
            # prints average MSE per epoch
            if len(x) > 0:
                avg = S_errors / len(x)
            else:
                avg = float('nan')
            print(f"Epoch {i + 1}/{epochs} - avg MSQE: {avg:.6f}")

    def GD(self, lr=0.01):  # Gradient descent
...
if __name__ == "__main__":
    ...
    with open('test_data.csv') as csv_file:
        ...
        sequence = [list(map(float, row)) for row in csv_reader]

    seq = np.asarray(sequence)  # shape (T, 24)
    print("Generated sequence shape:", seq.shape)

    if seq.shape[0] < 5:
        raise ValueError("Need at least 5 frames to create training inputs / targets")

    M = seq.shape[0] - 4  # number of samples
    # Assigning first 4 frames to inputs, 5th frame to targets
    training_inputs = np.hstack([seq[i:i + M] for i in range(4)])  # shape (M, 96)
    targets = seq[4:4 + M]  # shape (M, 24)

    print("Sequence-based 4-frame -> next-frame prediction:")
    print("  Sequence length:", len(sequence))
    print("  training_inputs.shape:", training_inputs.shape, "(4 consecutive frames)")
    print("  targets.shape:", targets.shape, "(next frame)")

    # create network with 96 inputs and 24 outputs
    nn = Full_NN(96, [128, 64, 32], 24)
    ...
    nn.train_nn(training_inputs, targets, 100, 0.015)  # trains the network and prints per-epoch MSQE

    # Testing data to identify if Network trained well.
    # Use a sample that matches the network input size (24).

    # For further testing, use newly generated data, putting it in the format of 96-element input array.

    n = random.randint(0, len(training_inputs) - 1)
    test_input = training_inputs[n]  # 1D array of length 96
    test_target = targets[n]  # corresponding 24-element target
...
print()
print("Neural Network actual output is ", NN_output, "\nthere is an error (not MSQE) of ", test_target - NN_output)
print("=================================================================")
